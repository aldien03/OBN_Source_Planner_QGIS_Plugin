...
from qgis.core import (
    ...
    QgsUnitTypes, QgsRuleBasedRenderer, QgsVectorLayerSimpleLabeling # Added imports
)
...
class OBNPlannerDockWidget(QtWidgets.QDockWidget, Ui_OBNPlannerDockWidgetBase):
    ...
    def _generate_direct_deviation(self, entry_point, exit_point, avoidance_geom, clearance_m):
        ...
        # Log call stack to identify LineNum in calling functions
        import inspect
        current_frame = inspect.currentframe()
        call_stack = inspect.getouterframes(current_frame)
        log.debug("===== CALL STACK FOR DEBUGGING =====")
        for i, frame_info in enumerate(call_stack[:5]):  # Limit to first 5 frames
            log.debug(f"Frame {i}: {frame_info.filename}:{frame_info.lineno} - {frame_info.function}")
        log.debug("====================================")
        ...
        log.debug(f"ENTRY POINT - Original: ({entry_point.x():.2f}, {entry_point.y():.2f}) | Safe: ({entry_point_safe.x():.2f}, {entry_point_safe.y():.2f})")
        log.debug(f"EXIT POINT - Original: ({exit_point.x():.2f}, {exit_point.y():.2f}) | Safe: ({exit_point_safe.x():.2f}, {exit_point_safe.y():.2f})")
        ...
        try:
            ...
            log.debug(f"ENTRY POINT GEOMETRY - WKT: {entry_point_geom.asWkt()} | Valid: {entry_point_geom.isGeosValid()} | Type: {entry_point_geom.wkbType()}")
            log.debug(f"EXIT POINT GEOMETRY - WKT: {exit_point_geom.asWkt()} | Valid: {exit_point_geom.isGeosValid()} | Type: {exit_point_geom.wkbType()}")
            ...
            log.debug(f"DIRECT LINE - WKT: {direct_line.asWkt()} | Valid: {direct_line.isGeosValid()} | Type: {direct_line.wkbType()} | Length: {direct_line.length():.2f}m")
            
            # Extract vertices from direct line for detailed inspection
            direct_vertices = direct_line.asPolyline()
            log.debug(f"DIRECT LINE VERTICES - Count: {len(direct_vertices)}")
            for i, vertex in enumerate(direct_vertices):
                log.debug(f"  Vertex {i}: ({vertex.x():.2f}, {vertex.y():.2f})")
            ...
            if avoidance_geom and not avoidance_geom.isEmpty():
                log.debug(f"NO-GO ZONE - WKT: {avoidance_geom.asWkt()[:200]}... | Valid: {avoidance_geom.isGeosValid()} | Type: {avoidance_geom.wkbType()} | Area: {avoidance_geom.area():.2f}")
                
                # Check intersection between direct line and avoidance geometry
                intersection = direct_line.intersection(avoidance_geom)
                if intersection and not intersection.isEmpty():
                    log.debug(f"INTERSECTION - EXISTS: Yes | WKT: {intersection.asWkt()[:200]}... | Type: {intersection.wkbType()}")
                    self._visualize_path(intersection, "intersection", QColor(255, 255, 0))  # Yellow
                else:
                    log.debug("INTERSECTION - EXISTS: No (direct line does not intersect no-go zone)")
        ...
        try:
            ...
            centroid = avoidance_geom.centroid().asPoint()
            log.debug(f"NO-GO ZONE CENTROID - Point: ({centroid.x():.2f}, {centroid.y():.2f})")
            centroid_geom = QgsGeometry.fromPointXY(centroid)
            self._visualize_path(centroid_geom, "centroid", QColor(0, 0, 255))  # Blue
            ...
            log.debug(f"DIRECT LINE MIDPOINT - Point: ({mid_point.x():.2f}, {mid_point.y():.2f})")
            log.debug(f"DIRECT LINE VECTOR - dx: {dx:.2f}, dy: {dy:.2f}")
            
            # Visualize midpoint
            mid_point_geom = QgsGeometry.fromPointXY(mid_point)
            self._visualize_path(mid_point_geom, "midpoint", QColor(255, 165, 0))  # Orange
            
            # Calculate perpendicular direction vectors
            length = math.sqrt(dx*dx + dy*dy)
            log.debug(f"DIRECT LINE LENGTH - {length:.2f}m")
            
            if length == 0:
                log.warning("ERROR: Entry and exit points are too close (zero distance)")
                return None
            ...
            log.debug(f"PERPENDICULAR VECTORS:")
            log.debug(f"  Vector 1 (normalized): ({perp1_x:.4f}, {perp1_y:.4f})")
            log.debug(f"  Vector 2 (normalized): ({perp2_x:.4f}, {perp2_y:.4f})")
            
            # Visualize the perpendicular vectors (scaled for visibility)
            scale = length * 0.25  # Scale to 25% of direct line length for visibility
            perp1_point = QgsPointXY(mid_point.x() + perp1_x * scale, mid_point.y() + perp1_y * scale)
            perp2_point = QgsPointXY(mid_point.x() + perp2_x * scale, mid_point.y() + perp2_y * scale)
            
            perp1_line = QgsGeometry.fromPolylineXY([mid_point, perp1_point])
            perp2_line = QgsGeometry.fromPolylineXY([mid_point, perp2_point])
            
            self._visualize_path(perp1_line, "perp_vector1", QColor(0, 128, 128))  # Teal
            self._visualize_path(perp2_line, "perp_vector2", QColor(128, 0, 128))  # Purple
            
            # Determine which side to go around (away from centroid)
            vec_to_centroid_x = centroid.x() - mid_point.x()
            vec_to_centroid_y = centroid.y() - mid_point.y()
            
            # Visualize vector to centroid
            centroid_vec_line = QgsGeometry.fromPolylineXY([mid_point, centroid])
            self._visualize_path(centroid_vec_line, "to_centroid_vector", QColor(255, 215, 0))  # Gold
            
            log.debug(f"VECTOR TO CENTROID - ({vec_to_centroid_x:.4f}, {vec_to_centroid_y:.4f})")
            
            dot1 = vec_to_centroid_x * perp1_x + vec_to_centroid_y * perp1_y
            dot2 = vec_to_centroid_x * perp2_x + vec_to_centroid_y * perp2_y
            
            log.debug(f"DOT PRODUCTS:")
            log.debug(f"  Dot1 (perp1 · vec_to_centroid): {dot1:.4f}")
            log.debug(f"  Dot2 (perp2 · vec_to_centroid): {dot2:.4f}")
            ...
            away_direction = perp1 if dot1 < dot2 else perp2
            toward_direction = perp2 if dot1 < dot2 else perp1
            directions = [away_direction, toward_direction]
            direction_names = ["away from centroid", "toward centroid"]
            
            log.debug(f"SELECTED DIRECTIONS:")
            log.debug(f"  Primary (away from centroid): ({away_direction[0]:.4f}, {away_direction[1]:.4f})")
            log.debug(f"  Secondary (toward centroid): ({toward_direction[0]:.4f}, {toward_direction[1]:.4f})")
